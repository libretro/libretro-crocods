...
{
public:
	CZipTool(unsigned char *buffer, unsigned int size);
	CZipTool(char *filename);
	~CZipTool();
	
	bool ReadFromZIP(char *filter, unsigned int *dsize, unsigned char **dBuffer);
...
protected:
	...
	void zipwrite( unsigned char* dat, int len );
	
	int zipread( unsigned char* dat, int len );
	...
	void Unstore( unsigned int usz, unsigned int csz );
	void Inflate( unsigned int usz, unsigned int csz );
	void Unshrink( unsigned int usz, unsigned int csz );

	// unreduce
	void Unreduce( unsigned int usz, unsigned int csz, int factor );
	void LoadFollowers( unsigned char* Slen, unsigned char followers[][64] );
	
	// explode
	void Explode( unsigned int usz, unsigned int csz, bool _8k,bool littree );
...
public:
	unsigned int crc32( unsigned int crc, const unsigned char* dat, int len );
...
CZipTool::CZipTool(unsigned char *buffer, unsigned int size)
{
	kzip=buffer;
	mzip=size;
...
int CZipTool::zipread( unsigned char* dat, int len )
{
	len = kread( dat, len);
	return len;
}
...
void CZipTool::zipwrite( unsigned char* dat, int len )
{
	kwrite( dat, len);
}
...
bool CZipTool::ReadFromZIP(char *filter, unsigned int *dsize, unsigned char **dBuffer)
{
	ZipLocalHeader hdr;
...
void CZipTool::Unstore( unsigned int usz, unsigned int csz )
{
...
void CZipTool::Inflate( unsigned int usz, unsigned int csz )
{
...
void CZipTool::Unshrink( unsigned int usz, unsigned int csz )
{
	// 8192Bytes = 13bits
...
void CZipTool::LoadFollowers( unsigned char* Slen, unsigned char followers[][64] )
{
	for( int x=255; x>=0; x-- )
	{
		Slen[x] = getbits(6);
...
void CZipTool::Unreduce( unsigned int usz, unsigned int csz, int factor )
{
	unsigned char* outbuf = common_buf;
...
void CZipTool::Explode( unsigned int usz, unsigned int csz, bool _8k, bool littree )
{
	unsigned char* outbuf = common_buf;
	unsigned char* outpos=outbuf;
	memset( outbuf,0,0x4000 );
