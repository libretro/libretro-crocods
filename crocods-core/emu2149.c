...
internal_refresh(PSG *psg)
{
    if (psg->quality) {
        ...
        psg->realstep = (u32)((1 << 31) / psg->rate);
        psg->psgstep = (u32)((1 << 31) / (psg->clk / 8));
        psg->psgtime = 0;
    } else {
        psg->base_incr =
            (u32)((double)psg->clk * (1 << GETA_BITS) / (8.0 * psg->rate));
    }
}

EMU2149_API void
PSG_set_clock(PSG *psg, u32 c)
{
    psg->clk = c;
...
EMU2149_API void
PSG_setStereoMask(PSG *psg, u32 mask)
{
//    u32 ret = 0;
    if (psg) {
...
EMU2149_API u32
PSG_toggleMask(PSG *psg, u32 mask)
{
    u32 ret = 0;
    if (psg) {
        ret = psg->mask;
...
PSG_readIO(PSG *psg)
{
    return (u8)(psg->reg[psg->adr]);
}

EMU2149_API u8
PSG_readReg(PSG *psg, u32 reg)
{
    return (u8)(psg->reg[reg & 0x1f]);
}

EMU2149_API void
PSG_writeIO(PSG *psg, u32 adr, u32 val)
{
    if (adr & 1) PSG_writeReg(psg, psg->adr, val);
    else psg->adr = val & 0x1f;
}

static s16 calc(PSG *psg)
{
    int i, noise;
    u32 incr;
    s32 mix = 0;
    ...
    while (psg->env_count >= 0x10000 && psg->env_freq != 0) {
        if (!psg->env_pause) {
            if (psg->env_face) psg->env_ptr = (psg->env_ptr + 1) & 0x3f;
            else psg->env_ptr = (psg->env_ptr + 0x3f) & 0x3f;
        }
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (psg->mask & PSG_MASK_CH(i)) continue;

        if ((psg->tmask[i] || psg->edge[i]) && (psg->nmask[i] || noise)) {
            if (!(psg->volume[i] & 32)) psg->cout[i] = psg->voltbl[psg->volume[i] & 31];
            else psg->cout[i] = psg->voltbl[psg->env_ptr];
...
PSG_calc(PSG *psg)
{
    if (!psg->quality) return (s16)(calc(psg) << 4);
    ...
    return (s16)(psg->out << 4);
}

static void
calc_stereo(PSG *psg, s32 out[2])
{
    int i, noise;
    u32 incr;
    s32 l = 0, r = 0;
    ...
    for (i = 0; i < 3; i++) {
        ...
        if ((psg->tmask[i] || psg->edge[i]) && (psg->nmask[i] || noise)) {
            ...
            if (psg->stereo_mask[i] & 0x01) l += psg->cout[i];
            if (psg->stereo_mask[i] & 0x02) r += psg->cout[i];
        }
    }
...
EMU2149_API void
PSG_calc_stereo(PSG *psg, s32 *bufMO, s32 *bufRO)
{
    s32 buffers[2];
    ...
    if (!psg->quality) {
        ...
        *bufMO = (s32)(((double)psg->snext[0] * (psg->psgstep - psg->psgtime)
                        + (double)psg->sprev[0] * psg->psgtime) / psg->psgstep);
        *bufRO = (s32)(((double)psg->snext[1] * (psg->psgstep - psg->psgtime)
                        + (double)psg->sprev[1] * psg->psgtime) / psg->psgstep);
    }
}

EMU2149_API void
PSG_writeReg(PSG *psg, u32 reg, u32 val)
{
    int c;
